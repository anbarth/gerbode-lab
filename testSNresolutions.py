...
if __name__ == '__main__':
    ...
    with open('polyScalingMar28.csv','w',newline='') as file:
        writer = csv.writer(file)
        crysfilename = 'crystals/readshock2/rs2p5_1.csv'
        writer.writerow([crysfilename])
        writer.writerow(['freeSpacePoly, inclusive distance. new isAvailPoly that doesnt ue matplotlib -- lets compare S/N and runtime! 40 worker processes. DrSpeedy'])
        writer.writerow(['rad','S_poly','N','runtime'])
        
        for radius in radii:
            coll = exclVol.PolycrystalGrid(crysfilename,rad=radius,useNeighbs=True)
            print(coll.beadRad)
            (Spoly,N,t) = coll.entropyParallel(40,makeImg=False,poly=True)
